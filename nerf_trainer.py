...
class NerfTrainer:
    ...
    def __init__(self, args, rank, local_rank, net_arch='nerf'):
        ...
        self.render_kwargs_train, self.render_kwargs_test, self.start, self.grad_vars, self.optimizer = self._create_nerf()
    ...
    def _create_nerf(self):
        ...
        skips = [4]
        self.model = NeRF(D=self.args.netdepth, W=self.args.netwidth,
        ...
        if self.args.N_importance > 0:
            self.model_fine = NeRF(D=self.args.netdepth_fine, W=self.args.netwidth_fine,
                              input_ch=input_ch, output_ch=output_ch, skips=skips,
                              input_ch_views=input_ch_views, use_viewdirs=self.args.use_viewdirs).cuda()
        ...
        # Create optimizer
        optimizer = Lamb(params=grad_vars, lr=self.args.lrate, betas=(0.9, 0.999))
    ...
    def train_one_epoch(self, epoch):
        ...
        # localize model hyper-parameters
        render_kwargs_train = self.render_kwargs_train
        render_kwargs_test = self.render_kwargs_test
        start = self.start
        grad_vars = self.grad_vars
        optimizer = self.optimizer

        # localize dataset parameters
        K = self.K
        images = self.images
        poses = self.poses
        bds = self.bds
        ...
        for i in trange(start, N_iters):
            ...
            rgb, disp, acc, extras = self.render(H, W, K, chunk=self.args.chunk, rays=batch_rays,
