...
class NerfTrainer:
    ...
    def __init__(self, args, rank, local_rank, nworkers=1, net_arch='nerf'):
        ...
        # initialization
        self.K, self.images, self.poses, self.bds, self.render_poses, self.i_train, self.i_test, self.i_val, self.hwf, self.poses, self.near, self.far = self._prepare_dataset()
        self.render_kwargs_train, self.render_kwargs_test, self.step, self.grad_vars, self.optimizer = self._create_nerf()
    ...
    def run_network(self, inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
        ...
        outputs_flat = self.batchify(fn, netchunk)(embedded)
    ...
    def named_parameters(self):
        """return the (name, parameters) of models.
           ...
           may have two models
        """
        model = self.render_kwargs_train["network_fn"]
        model_fine = self.render_kwargs_train["network_fine"]
    ...
    def _create_nerf(self):
        ...
        """
        embed_fn, input_ch = get_embedder(self.args.multires, self.args.i_embed)
    
        input_ch_views = 0
        embeddirs_fn = None
        if self.args.use_viewdirs:
            embeddirs_fn, input_ch_views = get_embedder(self.args.multires_views, self.args.i_embed)
        output_ch = 5 if self.args.N_importance > 0 else 4
        skips = [4]
        model = NeRF(D=self.args.netdepth, W=self.args.netwidth,
                     input_ch=input_ch, output_ch=output_ch, skips=skips,
                     input_ch_views=input_ch_views, use_viewdirs=self.args.use_viewdirs).cuda()
        grad_vars = list(model.parameters())
    
        model_fine = None
        if self.args.N_importance > 0:
            model_fine = NeRF(D=self.args.netdepth_fine, W=self.args.netwidth_fine,
                              input_ch=input_ch, output_ch=output_ch, skips=skips,
                              input_ch_views=input_ch_views, use_viewdirs=self.args.use_viewdirs).cuda()
        ...
        if self.args.dataset_type != 'llff' or self.args.no_ndc:
            ...
            render_kwargs_train['lindisp'] = self.args.lindisp
    ...
    def _prepare_dataset(self):
        ...
        return K, images, poses, bds, render_poses, i_train, i_test, i_val, hwf, poses, near, far
    ...
    def train_one_step(self, step):
        ...
        if self.use_batching:
            ...
            batch = self.rays_rgb[self.i_batch:self.i_batch+self.N_rand] # [B, 2+1, 3*?]
        ...
        rgb, disp, acc, extras = self.render(self.H, self.W, self.K, chunk=self.args.chunk, rays=batch_rays,
                                                verbose=step < 10, retraw=True,
    ...
    def log_test_set(self):
        ...
        with torch.no_grad():
            rgbs, _ = self.render_path(torch.Tensor(self.poses[self.i_test]).cuda(), self.hwf, self.K, self.args.chunk, self.render_kwargs_test, gt_imgs=self.images[self.i_test], savedir=testsavedir)
    ...
    def log_test_video(self):
        ...
        with torch.no_grad():
            rgbs, disps = self.render_path(self.render_poses, self.hwf, self.K, self.args.chunk, self.render_kwargs_test)
    ...
    def render_only(self):
        ...
        # TODO: export it as a single function
