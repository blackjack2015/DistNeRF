...
import torch
from models.ray_utils import Rays, convert_to_ndc, namedtuple_map
from models.pose_utils import normalize, look_at, poses_avg, recenter_poses, to_float, generate_spiral_cam_to_world, generate_spherical_cam_to_world, flatten
from torch.utils.data import Dataset, DataLoader
from dataloader.distributed import ImageDistributedSampler
...
def get_dataloader(dataset_name, base_dir, split, factor=4, batch_size=None, shuffle=True, device=torch.device("cpu"), nworkers=1, rank=0):
    ...
    if nworkers > 1:
        if split == 'train':
            sampler = torch.utils.data.distributed.DistributedSampler(
                d, num_replicas=nworkers, rank=rank, shuffle=shuffle)
        else:
            sampler = ImageDistributedSampler(
                d, d.h, d.w, num_replicas=nworkers, rank=rank, shuffle=shuffle)
        sampler.set_epoch(0)
        loader = MultiEpochsDataLoader(d, batch_size = batch_size, \
...
def ray_to_device(rays):
    return namedtuple_map(lambda r: r.cuda(), rays)
...
class NeRFDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.split == "render":
            ...
            # return ray_to_device(ray), pixel
            return ray, pixel
